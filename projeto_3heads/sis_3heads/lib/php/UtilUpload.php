...
class UtilUpload {
	...
	function gera_fotos($diretorio, $diretorio_g, $diretorio_p, $width_img, $height_img, $width_thumb, $height_thumb ) {
		...
		if ($aux_tipo_imagem == "image/jpeg") {
			...
			$this->nome_foto  = "imagem_".date("Ymd").time() . ".jpg"; // variavel para BD
			$this->nome_thumb = "thumb_".date("Ymd").time() . ".jpg"; // variavel para BD REDUZIDA

			//determino uma resolucao maxima e se a imagem for maior ela sera reduzida
			$this->reduz_imagem_jpg($this->arquivo['tmp_name'], $width_img, $height_img, $diretorio_g.$this->nome_foto);
			//passo o tamanho da thumbnail
			$this->reduz_imagem_jpg($this->arquivo['tmp_name'], $width_thumb, $height_thumb, $diretorio_p.$this->nome_thumb);
		}

		if ($aux_tipo_imagem == "image/gif") {
			$this->nome_foto  = "imagem_".date("Ymd").time().".gif";
			$this->nome_thumb = "thumb_".date("Ymd").time().".gif";
			//determino uma resolucao maxima e se a imagem for maior ela sera reduzida
			$this->reduz_imagem_gif($this->arquivo['tmp_name'],  $width_img, $height_img, $diretorio_g.$this->nome_foto);
			//passo o tamanho da thumbnail
			$this->reduz_imagem_gif($this->arquivo['tmp_name'],$width_thumb, $height_thumb, $diretorio_p.$this->nome_thumb);
		}

		if ($aux_tipo_imagem == "image/png") {
			$this->nome_foto  = "imagem_".date("Ymd").time().".png";
			$this->nome_thumb = "thumb_".date("Ymd").time().".png";
			//determino uma resolucao maxima e se a imagem for maior ela sera reduzida
			$this->reduz_imagem_png($this->arquivo['tmp_name'], $width_img, $height_img, $diretorio_g.$this->nome_foto);
			//passo o tamanho da thumbnail
			$this->reduz_imagem_png($this->arquivo['tmp_name'], $width_thumb, $height_thumb, $diretorio_p.$this->nome_thumb);
		}
